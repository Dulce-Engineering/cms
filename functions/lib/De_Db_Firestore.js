...
class De_Db_Firestore
{
  ...
  async Select_Objs_By_Ids(ids, table_name, class_type)
  {
    ...
    if (!De_Db_Firestore.Is_Empty(ids))
    {
      objs = [];
      for (const id of ids)
    {
  ...
  async Update(class_obj, table_name)
  {
    ...
    //console.log("De_Db_Firestore.Update(): res =", res);
    return res;
  }
        ...
        if (idx < order_bys.length)
  {
          ...
          //console.log("De_Db_Firestore.Order_By(): order_by =", order_by);
          const compare_dir = order_by.dir == "asc" ? 1: -1;
          const value_a = a[order_by.field];
          const value_b = b[order_by.field];
          //console.log("De_Db_Firestore.Order_By(): value_a, value_b =", value_a, value_b);
