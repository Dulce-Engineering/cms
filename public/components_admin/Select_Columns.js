...
class Select_Columns extends HTMLElement 
{
  ...
  set value(col_items)
  {
    if (col_items && col_items.length > 0)
    {
      for (const column of col_items)
      {
        ...
        this[col_btn_id].checked = column.visible;
      }
  ...
  {
    // show field menu
    this.column_list.classList.add(this.edit_class);
  }
  ...
  Render()
  {
    ...
    const html = `
      <div class="header">
        Edit Columns: 
        <span class="edit_banner">
          <button id="edit_btn" class="btn edit_btn">✎</button>
          <ul id="column_list" class="column_list">
            <li class="hdr_item">
